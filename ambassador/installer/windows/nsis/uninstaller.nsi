...
Section "Uninstall"
  ...
  StrCpy $0 "Software\Clients\IRC\${AppRegName}\shell\open\command"
  ReadRegStr $R1 HKLM "$0" ""
  ${un.RemoveQuotesFromPath} "$R1" $R1
  ${un.GetParent} "$R1" $R1

  ; Only remove the Clients\IRC key if it refers to this install location.
  ; The Clients\IRC registry key is independent of the default app for the OS
  ...
  ${If} "$INSTDIR" == "$R1"
    DeleteRegKey HKLM "Software\Clients\IRC\${AppRegName}"
